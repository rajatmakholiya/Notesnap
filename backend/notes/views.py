...
def create_summarised_notes(request):
    ...
    transcription = client.audio.transcriptions.create(
        model="whisper-1",
        file=open(uploaded_file.temporary_file_path(), "rb")
    )

    completion = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
            {"role": "user", "content": f"Assume the persona of a teacher and summarise the below notes for me, with proper formatting:\n{transcription.text}"}
        ]
    )
    with open(dest_file_name, "w") as destination_file:
        destination_file.write(completion.choices[0].message.content)
